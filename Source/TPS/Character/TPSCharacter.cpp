...
ATPSCharacter::ATPSCharacter()
{
	...
	SpringArm->SetupAttachment(RootComponent); // Привязка к корневому компоненту
	SpringArm->TargetArmLength = 300.0f; // Дистанция до камеры
	SpringArm->bUsePawnControlRotation = true; // Включаем вращение для Spring Arm
	SpringArm->bEnableCameraLag = true;
	SpringArm->CameraLagSpeed = 3.0f;

	// Создание камеры
	Camera = CreateDefaultSubobject<UCameraComponent>(TEXT("Camera"));
	Camera->SetupAttachment(SpringArm); // Привязка камеры к Spring Arm
	Camera->bUsePawnControlRotation = false; // Отключаем вращение камеры
}
